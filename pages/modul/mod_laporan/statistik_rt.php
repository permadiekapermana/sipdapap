...
if (!empty($query))
	...
	$x=$pdf->GetY();
	$pdf->SetY($x+1);
	$pdf->Cell(0,1,'Data Kependudukan Menurut RT / RW',0,0,'C');
...
while ($rt=mysql_fetch_array($baca))
{
	// $limited_string = limit_words($agama[agama], 2);
	...
	$pdf->Cell(1,1,$no,1,0,'C');
	$pdf->Cell(4,1,$rt[rt].' / '.$rt[rw],1,0,'C');
	$pdf->Cell(4,1,$j_rt_l,1,0,'C');
	$pdf->Cell(3,1,$pros_rt_l.'%',1,0,'C');
	$pdf->Cell(4,1,$j_rt_p,1,0,'C');
	$pdf->Cell(3,1,$pros_rt_p.'%',1,0,'C');
	$pdf->Cell(4,1,$j_tot_rt,1,0,'C');
	$pdf->Cell(4,1,$j_pros_rt.'%',1,0,'C');
	...
	$pdf->SetFont('Arial','',12);
	//$pdf->Cell(2.2,1,$hasil[kode_buku],1,0,'C');
	$pdf->Cell(5,1,'Total Keseluruhan',1,0,'C');
