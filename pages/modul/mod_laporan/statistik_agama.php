...
if (!empty($query))
	...
	$x=$pdf->GetY();
	$pdf->SetY($x+1);
	$pdf->Cell(0,1,'Data Kependudukan Menurut Agama',0,0,'C');
...
while ($agama=mysql_fetch_array($baca))
{
	...
	$pdf->Cell(1,1,$no,1,0,'C');
	$pdf->Cell(4,1, $agama[agama] ,1,0,'C');
	$pdf->Cell(4,1,$j_agama_l,1,0,'C');
	$pdf->Cell(3,1,$pros_agama_l.'%',1,0,'C');
	$pdf->Cell(4,1,$j_agama_p,1,0,'C');
	$pdf->Cell(3,1,$pros_agama_p.'%',1,0,'C');
	$pdf->Cell(4,1,$j_tot_agama,1,0,'C');
	$pdf->Cell(4,1,$j_pros_agama.'%',1,0,'C');
