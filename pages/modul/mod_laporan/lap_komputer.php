...
$query= "SELECT *  FROM komputer ORDER BY kd_komputer ASC";
...
if (!empty($query))
	...
	$pdf->Ln();
	$pdf->Cell(0,.6,'HEKSA COMPUTER ',0,0,'C');
	$pdf->Ln();
	$pdf->SetFont('Arial','B',14);
	$pdf->Cell(0,.6,'METODE FUZZY LOGIC',0,0,'C');
	$pdf->Ln();
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(0,.6,'Office : Sumber, Cirebon',0,0,'C');
	$pdf->Ln();
	$pdf->Cell(0,.6,'web : http:// e_mail: amran.hamidy@yahoo.com',0,0,'C');	
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
	...
	$x=$pdf->GetY();
	$pdf->SetY($x+1);
	$pdf->Cell(0,1,'Daftar Komputer ',0,0,'C');
	...
	$pdf->Cell(5,1,'NAMA KOMPUTER',1,0,'C');
	$pdf->Cell(5,1,'SPESIFIKASI',1,0,'C');
...
while ($hasil=mysql_fetch_array($baca))
{
	...
	$pdf->Cell(2,1,$no,1,0,'C');
	$pdf->Cell(5,1,$hasil['nm_komputer'],1,0,'C');
	$pdf->Cell(5,1,$hasil['spesifikasi'],1,0,'C');
	...
	$pdf->Ln();
	$pdf->Cell(15,0.5,'');
	$pdf->Cell(0,0.5,'HEKSA COMPUTER ',0,0,'C');
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Cell(15,0.5,'');
	$pdf->Cell(0,0.5,'Amran Hamidy',0,0,'C');
	$pdf->Ln();
	$pdf->Cell(15,0.5,'');
