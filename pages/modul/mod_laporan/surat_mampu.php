...
if (!empty($query))
	...
	$pdf->SetY($x+1);
	$pdf->SetFont('Arial','B',14);
	$pdf->Cell(0,1,'SURAT KETERANGAN TIDAK MAMPU (SKTM)',0,0,'C');
	$pdf->Ln();
	...
	$pdf->Ln();
		...
		$pdf->Ln();
		$pdf->Cell(2,1,'',2,0,'L');
		$pdf->Cell(3,1,'Kewarganegaraan',0,0,'L');
		$pdf->Cell(2,1,':',2,0,'C');
		$pdf->Cell(3,1,'Indonesia',2,0,'L');
		$pdf->Ln();
		$pdf->Cell(2,1,'',2,0,'L');
		...
		$pdf->Ln();
		$pdf->Cell(1,1,'',2,0,'L');
		$pdf->Cell(3,1,'Adalah benar yang bersangkutan warga Desa kami dan termasuk Keluarga Miskin.',0,0,'L');
		$pdf->Ln();
		$pdf->Cell(3,1,' Surat Keterangan ini dibuat untuk persyaratan : Keringanan Pembayaran di Rumah Sakit.',0,0,'L');
		$pdf->Ln();
		$pdf->Cell(3,1,'Demikian Surat Keterangan ini dibuat kepada yang bersangkutan agar menjadi tahu',0,0,'L');
		$pdf->Ln();
		$pdf->Cell(3,1,'dan maklum adanya',0,0,'L');
		$pdf->Ln();
		$pdf->Ln();
		$pdf->Ln();
