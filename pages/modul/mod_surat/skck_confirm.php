...
if (!empty($query))
  ...
  //   $r99    = mysql_fetch_array($edit9);
	...
	// 	$pdf->Ln();	
		...
		$pdf->Cell(2,1,'',2,0,'L');
		$pdf->Cell(3,1,'Adalah benar yang bersangkutan merupakan warga dari desa kami dan telah memohon keterangan',0,0,'L');
		$pdf->Ln();
		$pdf->Cell(3,1,'pengantar Catatan Kepolisian kepada kami dalam rangka persyaratan : '.$r1['isi_field'].'.',0,0,'L');
		}
		$pdf->Ln();
		$pdf->Cell(3,1,'Keterangan ini kami berikan kepadanya dengan berdasarkan sepengetahuan dan petimbangan bahwa :',0,0,'L');
		$pdf->Ln();
		$pdf->SetFont('Times','B',12);
		$pdf->Cell(2,1,'',2,0,'L');
		$pdf->Cell(3,1,'Benar orang tersebut adalah penduduk Desa kami yang belum tercatat dalam Kepolisian.',0,0,'L');
		$pdf->SetFont('Times','',12);
		$pdf->Ln();
		$pdf->Cell(3,1,'Surat keterangan ini berlaku sampai urusan yang bersangkutan selesai.',0,0,'L');
		$pdf->Ln();
		$pdf->Cell(3,1,'Demikian, surat keterangan ini dibuat agar dapat dipergunakan sebagaimana mestinya.',0,0,'L');
