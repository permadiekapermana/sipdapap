...
if (!empty($query))
	...
	$pdf->Cell(3,1,'Yang bertanda tangan di bawah ini Kepala Desa Sitiwinangun Kecamatan Jamblang Kabupaten',0,0,'L');
		...
		$pdf->Cell(2,1,':',2,0,'C');
		$pdf->Cell(3,1,$r['tmp_lahir']." , 22-01-1990",2,0,'L');
		$pdf->Ln();
		$pdf->Cell(2,1,'',2,0,'L');
		...
		$pdf->Cell(2,1,'',2,0,'L');
		$pdf->Cell(3,1,'Adalah benar yang bersangkutan merupakan warga dari desa kami dan termasuk keluarga miskin.',0,0,'L');
		$pdf->Ln();
		$pdf->Cell(3,1,'Surat Keterangan ini dibuat untuk tujuan : Berobat Rumah Sakit.',0,0,'L');
		
		$pdf->Ln();
		$pdf->Cell(3,1,'Demikian surat keterangan tidak mampu ini kami buat agar menjadi tahu dan maklum adanya dan dapat',0,0,'L');
		$pdf->Ln();
