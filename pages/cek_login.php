...
$pass     = anti_injection(sha1($_POST['password']));
...
if ($ketemu > 0){
  ...
  if ($r[level]=='Penduduk'){
    $edit = mysql_query("SELECT * FROM penduduk WHERE id_penduduk='$r[id_penduduk]'");
    $r2    = mysql_fetch_array($edit); 

    $_SESSION[namauser]     = $r[username];
  $_SESSION[id_user]  = $r[id_user];
    $_SESSION[namalengkap]  = $r2[nama];
  $_SESSION[passuser]     = $r[password];
  $_SESSION[leveluser]    = $r[level];
  $_SESSION[id_penduduk]    = $r[id_penduduk];
  }
  elseif ($r[level]=='Ketua RT'){
    $edit = mysql_query("SELECT * FROM penduduk WHERE id_penduduk='$r[id_penduduk]'");
    $r2    = mysql_fetch_array($edit); 
  ...
  $_SESSION[leveluser]    = $r[level];
  $_SESSION[id_penduduk]    = $r[id_penduduk];
  $_SESSION[id_rt]    = $r2[id_rt];
  }
  elseif ($r[level]=='Ketua RW'){
    $edit = mysql_query("SELECT
    *
  FROM
    `ketuarw`
    INNER JOIN `penduduk` ON `penduduk`.`id_penduduk` = `ketuarw`.`id_penduduk`
  WHERE
    `ketuarw`.`id_penduduk` = '$r[id_penduduk]'");
    $r2    = mysql_fetch_array($edit); 
  ...
  $_SESSION[leveluser]    = $r[level];
  $_SESSION[id_penduduk]    = $r[id_penduduk];
  $_SESSION[id_rw]    = $r2[id_rw];
  }
  elseif ($r[level]=='Kepala Desa' OR $r[level]=='Admin' OR $r[level]=='Operator'){
    $edit = mysql_query("SELECT * FROM perangkat_desa WHERE id_user='$r[id_user]'");
    $r2    = mysql_fetch_array($edit); 

    $_SESSION[namauser]     = $r[username];
  $_SESSION[id_user]  = $r[id_user];
  $_SESSION[id_perangkatdesa]  = $r[id_perangkatdesa];
    $_SESSION[namalengkap]  = $r2[nama];
  $_SESSION[passuser]     = $r[password];
  $_SESSION[leveluser]    = $r[level];
  }
